...
- la tercera cosa que puede darse es que el nombre sea un nombre compuesto, y que la primera parte sea compartida por otro fabricante:
    ...
    En caso de ser asi lo que sucede es que el contenido correspondiente al valor del diccionario se asigna en primer lugar a una variable llamada "compare" y a una lista temporal.
    De esta manera cuando el for vuelve a leer el texto que a nosotros nos interesa y comprueba si esa palabra existe en compare, si es afirmativo lo que pasara sera que la palabra
    detectada la meteremos en la lista temporal

    a continuacion mediante otro for se recorre la lista temporal para "transformar" las palabras que se han encontrado en una unica cadena de texto
    una vez finalizado el nombre forma parte de una variable de tipo texto que podemos meter en la lista correspondiente de las marcas.
